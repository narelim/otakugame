...
- [ ] **행사 당일 연출** — 부.꾸 스테이지 렌더러를 읽기 전용으로 추출해 행사 화면 배경 = 내가 꾸민 부스. 손님 스프라이트·말풍선·구매 연출, 오픈→피크→마감 페이즈, 판매된 굿즈가 진열에서 빠짐
...
- [ ] **성향 통계 추적** — 행동/지출 카테고리 누적 → 회고록·성향 칭호·도전과제의 데이터 소스 (위 콘텐츠들보다 먼저 깔아두면 좋음)
